...
def summarise(samples: list[list[dict]]) -> list[dict]:
    """Collapse repeated rounds into a per-endpoint reliability picture.

    This is the number that actually decides whether a source can back an archive of record.
    A single 200 proves a source is REACHABLE; only a success RATE across time proves it is
    RELIABLE, and the two were measured to differ sharply here -- the same pbpstats endpoint
    returned 200 in 2.6s on one round and timed out on the next.
    """
    ...
    for name, rs in by_name.items():
        ...
        for r in rs:
            if r["status"] != 200:
                key = str(r["status"]) if r["status"] else r["shape"].split(":")[0]
        ...
        out.append({
            ...
            "median_seconds": lat[len(lat) // 2] if lat else None,
            "max_seconds": max(lat) if lat else None,
            "errors": errs,
        })
    return sorted(out, key=lambda d: (-(d["success_rate"] or 0), d["name"]))


def main(argv: list[str] | None = None) -> int:
    import argparse

    ap = argparse.ArgumentParser(prog="probe-granular-sources", description=__doc__)
    ap.add_argument("--samples", type=int, default=1,
                    help="Rounds to run. >1 measures RELIABILITY, not just reachability.")
    ap.add_argument("--interval", type=float, default=300.0,
                    help="Seconds between rounds. Spread them out: back-to-back rounds measure a "
                         "moment, not a day.")
    ap.add_argument("--only", default=None, help="Substring filter on the source name")
    a = ap.parse_args(argv)

    sources = [s for s in SOURCES if not a.only or a.only.lower() in s[0].lower()]
    rounds = []
    for i in range(max(1, a.samples)):
        if i:
            print(f"PROBE_ROUND sleeping {a.interval}s before round {i + 1}")
    ...
    if a.samples > 1:
        print("PROBE_RELIABILITY " + json.dumps(summarise(rounds), indent=2))
    return 0
