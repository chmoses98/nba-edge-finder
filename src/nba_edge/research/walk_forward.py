...
def run(seasons: list[str], max_games: int, n_sims: int, warmup_games: int, hist_root: Path, out_path: Path, seed: int = 11, team_half_life: float = 25.0, team_prior_games: float = 4.0, rating_scale: float = 1.9) -> dict:
    ...
    for i, (_, r) in enumerate(eval_rows.iterrows()):
        ...
        gp, rep = build_game_params(game, tg, pg, date, {h: {}, a: {}}, None, BuildConfig(team_half_life=team_half_life, team_prior_games=team_prior_games, rating_scale=rating_scale))
    ...
    y = df["y"].to_numpy()
    rep = {"n": int(len(df)), "seasons": seasons, "n_sims": n_sims, "date_range": [df["date"].min(), df["date"].max()], "team_half_life": team_half_life, "team_prior_games": team_prior_games, "rating_scale": rating_scale}
